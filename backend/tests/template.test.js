  ...
  describe('Template Suite Tests', () => {
    ...
    describe('Template CRUD operations', () => {
      ...
      it('Get templates (existing templates in db)', async () => {
        const expected = [{ name: 'Template 1' }, { name: 'Template 2' }];
        var response = await request(app)
          .get('/api/templates')
          .set('Cookie', [`token=JWT ${userToken}`]);
      ...
      it('Update template with nonexistent ID', async () => {
        ...
        var response = await request(app)
          .put(`/api/templates/deadbeefdeadbeefdeadbeef`)
          .set('Cookie', [`token=JWT ${userToken}`])
          .send(template);
      ...
      it('Delete template', async () => {
        var response = await request(app)
          .delete(`/api/templates/${template2Id}`)
          .set('Cookie', [`token=JWT ${userToken}`]);
        expect(response.status).toBe(200);
      ...
      it('Delete template with nonexistent ID', async () => {
        var response = await request(app)
          .delete(`/api/templates/deadbeefdeadbeefdeadbeef`)
          .set('Cookie', [`token=JWT ${userToken}`]);
        expect(response.status).toBe(404);
      });
    });
