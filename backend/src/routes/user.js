module.exports = function(app) {
    ...
    app.post("/api/users/token", function(req, res) {
        ...
        .then(msg => {
            res.cookie('token', `JWT ${msg.token}`, {secure: true, httpOnly: true, sameSite: 'None'})
            res.cookie('refreshToken', msg.refreshToken, {secure: true, httpOnly: true, path: '/api/users/refreshtoken', sameSite: 'None'})
            Response.Ok(res, msg)
        })
        .catch(err => Response.Internal(res, err))
    });
